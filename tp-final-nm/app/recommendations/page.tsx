  ...
  if (!user) {
    return (
      <main className="min-h-screen flex flex-col items-center justify-center gap-4 px-6">
        ...
        <Link href="/" className="mt-2 px-5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium transition">
          Retour à l&apos;accueil
        </Link>
      </main>
    );
    ...
    return (
      <main className="min-h-screen flex flex-col items-center justify-center gap-4 px-6 text-center">
        ...
        <p className="text-gray-400 max-w-sm">
          Likez des films, séries, animes ou livres depuis la page de recherche pour obtenir des recommandations personnalisées.
        </p>
        <Link href="/" className="mt-2 px-5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium transition">
          Explorer du contenu
        </Link>
      ...
      <section className="mt-16 border-t border-gray-700 pt-10">
        ...
        {!selectedUser && (
          <div className="relative max-w-md">
            ...
            {searchLoading && (
              <p className="text-gray-500 text-xs mt-2 ml-1">Recherche…</p>
            )}
        ...
        {selectedUser && (
          <div className="max-w-2xl">
            <div className="flex items-center gap-3 mb-6 bg-gray-800/60 border border-gray-700 rounded-xl px-4 py-3">
              ...
              <button
                onClick={() => { setSelectedUser(null); setCompatibility(null); }}
            ...
            {compareLoading && (
              <p className="text-gray-400 text-sm animate-pulse">Calcul de la compatibilité…</p>
            )}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {compatibility.compatibility.commonGenres.map((g) => (
                          <span key={g} className="text-xs px-2 py-1 rounded-full bg-violet-900/50 text-violet-300 border border-violet-500/30">
                            {g}
                          </span>
                        ))}
                      </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {compatibility.compatibility.commonActors.map((a) => (
                          <span key={a} className="text-xs px-2 py-1 rounded-full bg-violet-900/50 text-violet-300 border border-violet-500/30">
                            {a}
                          </span>
                        ))}
                      </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {compatibility.compatibility.commonDirectors.map((d) => (
                          <span key={d} className="text-xs px-2 py-1 rounded-full bg-violet-900/50 text-violet-300 border border-violet-500/30">
                            {d}
                          </span>
                        ))}
                      </div>
