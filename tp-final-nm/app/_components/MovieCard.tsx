...
export default function MovieCard({ movie }: MovieCardProps) {
  return (
    <div className="relative group overflow-hidden flex flex-col w-64 flex-shrink-0">
      <Link
        href={`/details/movie/${movie.id}`}
        className="relative w-64 h-96 rounded-xl overflow-hidden shadow-2xl group cursor-pointer flex-shrink-0 block"
      >
        {/* Background image */}
        <Image
          src={movie.image}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4">
          ...
          <h3 className="text-white font-bold text-lg leading-tight mt-1 mb-1">{movie.title}</h3>
          <p className="text-gray-300 text-xs mb-2">
            Réalisé par <span className="text-amber-400">{movie.director}</span>
          </p>
          <p className="text-gray-400 text-xs line-clamp-2 leading-relaxed">{movie.description}</p>
